...
class anggota extends perpus{
	public function create($nim,$nama,$tempat_lahir){
		try {
		$stmt = $this->conn->prepare('INSERT INTO tbl_anggota(nim,nama,tempat_lahir) VALUES(?,?,?)');
	...
	public function update($nim,$nim1,$nama,$tempat_lahir){
		try {
		  $stmt = $this->conn->prepare("UPDATE tbl_anggota SET nim=:nim1, nama=:nama, tempat_lahir=:tempat_lahir WHERE nim=:nim");
